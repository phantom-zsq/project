package com.phantom.spark.sparkproject.meituan.spark.session;

import org.apache.spark.AccumulatorParam;

import com.phantom.spark.sparkproject.meituan.constant.Constants;
import com.phantom.spark.sparkproject.meituan.util.StringUtils;

/**
 * session聚合统计Accumulator
 * 
 * 大家可以看到 其实使用自己定义的一些数据格式，比如String，甚至说，我们可以自己定义model，自己定义的类（必须可序列化）
 * 然后呢，可以基于这种特殊的数据格式，可以实现自己复杂的分布式的计算逻辑
 * 各个task，分布式在运行，可以根据你的需求，task给Accumulator传入不同的值 根据不同的值，去做复杂的逻辑
 * 
...
public class SessionAggrStatAccumulator implements AccumulatorParam<String> {
	...
	/**
	 * zero方法，其实主要用于数据的初始化 那么，我们这里，就返回一个值，就是初始化中，所有范围区间的数量，都是0
	...
	public String zero(String v) {
		return Constants.SESSION_COUNT + "=0|" + Constants.TIME_PERIOD_1s_3s
				...
				+ Constants.TIME_PERIOD_30m + "=0|" + Constants.STEP_PERIOD_1_3
				+ "=0|" + Constants.STEP_PERIOD_4_6 + "=0|"
	...
	private String add(String v1, String v2) {
		...
		if (oldValue != null) {
			...
			return StringUtils.setFieldInConcatString(v1, "\\|", v2,

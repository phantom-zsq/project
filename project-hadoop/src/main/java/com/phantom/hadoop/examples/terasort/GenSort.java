...
public class GenSort {
	...
	static void generateRecord(byte[] recBuf, Unsigned16 rand, Unsigned16 recordNumber) {
		...
		/*
		 * convert the 128-bit record number to 32 bits of ascii hexadecimal as
		 * the next 32 bytes of the record.
		 */
		for (int i = 0; i < 32; i++) {
		...
		for (int i = 0; i < 12; ++i) {
			recBuf[48 + i * 4] = recBuf[49
					+ i * 4] = recBuf[50 + i * 4] = recBuf[51 + i * 4] = (byte) rand.getHexDigit(20 + i);
		}
	...
	static void generateAsciiRecord(byte[] recBuf, Unsigned16 rand, Unsigned16 recordNumber) {
		...
		for (int i = 0; i < 13; ++i) {
			recBuf[46 + i * 4] = recBuf[47
					+ i * 4] = recBuf[48 + i * 4] = recBuf[49 + i * 4] = (byte) rand.getHexDigit(19 + i);
		}

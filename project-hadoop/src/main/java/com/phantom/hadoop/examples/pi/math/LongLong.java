...
class LongLong {
	...
	static LongLong multiplication(final LongLong r, final long a, final long b) {
		/*
		 * final long x0 = a & LOWER_MASK; final long x1 = (a & UPPER_MASK) >>
		 * MID;
		 * 
		 * final long y0 = b & LOWER_MASK; final long y1 = (b & UPPER_MASK) >>
		 * MID;
		 * 
		 * final long t = (x0 + x1)*(y0 + y1); final long u = (x0 - x1)*(y0 -
		 * y1); final long v = x1*y1;
		 * 
		 * final long tmp = (t - u)>>>1; result.d0 = ((t + u)>>>1) - v + ((tmp
		 * << MID) & FULL_MASK);; result.d1 = v + (tmp >> MID); return result;
		 */

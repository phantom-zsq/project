...
import com.phantom.hadoop.examples.pi.Combinable;
...
public class Summation implements Container<Summation>, Combinable<Summation> {
	...
	public static Summation valueOf(final String s) {
		...
		if (s.length() > i) {
			...
			sigma.setValue(value.indexOf('.') < 0 ? Double.longBitsToDouble(Long.parseLong(value))
	...
	public Summation combine(Summation that) {
		...
		if (this.E.limit == that.E.value && this.N.limit == that.N.value) {
			...
			final Summation s = new Summation(new ArithmeticProgression(N.symbol, N.value, N.delta, that.N.limit),
	...
	public <T extends Container<Summation>> List<Summation> remainingTerms(List<T> sorted) {
		...
		if (sorted != null)
			for (Container<Summation> c : sorted) {
				...
				final Summation s = new Summation(sigma.N.limit, N.delta, remaining.N.limit, sigma.E.limit, E.delta,
						remaining.E.limit);
				if (s.getSteps() > 0)
					results.add(s);
				remaining = new Summation(remaining.N.value, N.delta, sigma.N.value, remaining.E.value, E.delta,
						sigma.E.value);
			}
	...
	public Summation[] partition(final int nParts) {
		...
		for (int i = 1; i < parts.length; i++) {
			...
			parts[i - 1] = new Summation(new ArithmeticProgression(N.symbol, prevN, N.delta, currN),
		...
		parts[parts.length - 1] = new Summation(new ArithmeticProgression(N.symbol, prevN, N.delta, N.limit),

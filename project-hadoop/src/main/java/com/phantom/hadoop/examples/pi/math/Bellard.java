...
import com.phantom.hadoop.examples.pi.Container;
import com.phantom.hadoop.examples.pi.Util;

/**
 * Bellard's BBP-type Pi formula 1/2^6 \sum_{n=0}^\infty (-1)^n/2^{10n}
 * (-2^5/(4n+1) -1/(4n+3) +2^8/(10n+1) -2^6/(10n+3) -2^2/(10n+5) -2^2/(10n+7)
 * +1/(10n+9))
 * 
 * References:
 *
...
public final class Bellard {
	...
	public enum Parameter {
		...
		// )
		P8_1(false, 1, 8, -1), P8_3(false, 3, 8, -6), P8_5(P8_1), P8_7(P8_3),

		/*
		 * 2^d\sum_{k=0}^\infty (-1)^k( 2^{ 2-10k} / (10k + 1) -2^{ -10k} / (10k
		 * + 3) -2^{-4-10k} / (10k + 5) -2^{-4-10k} / (10k + 7) +2^{-6-10k} /
		 * (10k + 9) )
		 */
		P20_21(true, 1, 20, 2), P20_3(false, 3, 20, 0), P20_5(false, 5, 20, -4), P20_7(false, 7, 20, -4), P20_9(true, 9,
				20, -6), P20_11(P20_21), P20_13(P20_3), P20_15(P20_5), P20_17(P20_7), P20_19(P20_9);
	...
	public static class Sum implements Container<Summation>, Iterable<Summation> {
		...
		private static <T extends Container<Summation>> Summation[] partition(Summation sigma, int nParts,
				List<T> existing) {
		...
		public String toString() {
			...
			for (Summation s : parts)
				if (s.getValue() == null)
					n++;
			return getClass().getSimpleName() + "{" + parameter + ": " + sigma + ", remaining=" + n + "}";
		}
		...
		public void setValue(Summation s) {
			...
			if (!s.contains(sigma) || !sigma.contains(s))
				throw new IllegalArgumentException(
						"!s.contains(sigma) || !sigma.contains(s)" + "\n  sigma=" + sigma + "\n  s    =" + s);
	...
	public static <T extends Container<Summation>> Map<Parameter, Sum> getSums(long b, int partsPerSum,
			Map<Parameter, List<T>> existing) {
	...
	public static <T extends Container<Summation>> double computePi(final long b, Map<Parameter, T> results) {
		if (results.size() != Parameter.values().length)
			throw new IllegalArgumentException(
					"m.size() != Parameter.values().length" + ", m.size()=" + results.size() + "\n  m=" + results);

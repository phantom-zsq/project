...
/**
 * The main class for computing sums using map/reduce jobs. A sum is partitioned
 * into jobs. A job may be executed on the map-side or on the reduce-side. A
 * map-side job has multiple maps and zero reducer. A reduce-side job has one
 * map and multiple reducers. Depending on the clusters status in runtime, a
 * mix-type job may be executed on either side.
 */
public final class DistSum extends Configured implements Tool {
	...
	static class Parameters {
		...
		static Parameters parse(String[] args, int i) {
			if (args.length - i < COUNT)
				throw new IllegalArgumentException("args.length - i < COUNT = " + COUNT + ", args.length=" + args.length
						+ ", i=" + i + ", args=" + Arrays.asList(args));
			...
			return new Parameters(
					"m".equals(type) ? MapSide.INSTANCE : "r".equals(type) ? ReduceSide.INSTANCE : MixMachine.INSTANCE,
	...
	public static abstract class Machine {
		...
		/** Compute sigma */
		static void compute(Summation sigma, TaskInputOutputContext<?, ?, NullWritable, TaskResult> context)
		...
		public static abstract class AbstractInputFormat extends InputFormat<NullWritable, SummationWritable> {
			...
			@Override
			public final RecordReader<NullWritable, SummationWritable> createRecordReader(InputSplit generic,
	...
	public static class MapSide extends Machine {
		...
		public static class PartitionInputFormat extends AbstractInputFormat {
			/**
			 * Partitions the summation into parts and then return them as
			 * splits
			 */
			@Override
	...
	public static class ReduceSide extends Machine {
		...
		public static class SummingReducer extends Reducer<IntWritable, SummationWritable, NullWritable, TaskResult> {
			@Override
			protected void reduce(IntWritable index, Iterable<SummationWritable> sums, Context context)
					throws IOException, InterruptedException {
	...
	/////////////////////////////////////////////////////////////////////////////
	/**
	 * A machine which chooses Machine in runtime according to the cluster
	 * status
	 */

 ...
 * of Pi. This program is able to calculate digit positions lower than a certain
 * limit, which is roughly 10^8. If the limit is exceeded, the corresponding
 * results may be incorrect due to overflow errors. For computing higher bits of
 * Pi, consider using distbbp.
 * 
 * Reference:
 *
 * [1] David H. Bailey, Peter B. Borwein and Simon Plouffe. On the Rapid
 * Computation of Various Polylogarithmic Constants. Math. Comp., 66:903-913,
 * 1996.
 */
public class BaileyBorweinPlouffe extends Configured implements Tool {
	...
	public static class BbpMapper extends Mapper<LongWritable, IntWritable, LongWritable, BytesWritable> {
		...
		protected void map(LongWritable offset, IntWritable length, final Context context)
	...
	public static class BbpReducer extends Reducer<LongWritable, BytesWritable, LongWritable, BytesWritable> {
		...
		@Override
		protected void reduce(LongWritable offset, Iterable<BytesWritable> values, Context context)
				throws IOException, InterruptedException {
			// read map outputs
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			...
			if (conf.getInt(DIGIT_START_PROPERTY, 1) == 1) {
				...
				try {
					...
					final int decDigits = 2 * hex.size(); // TODO: this is
															// conservative.
	...
	public static class BbpSplit extends InputSplit implements Writable {
		...
		private BbpSplit(int i, long offset, int size) {
			LOG.info("Map #" + i + ": workload=" + workload(offset, size) + ", offset=" + offset + ", size=" + size);
			this.offset = offset;
			this.size = size;
		}
	...
	/**
	 * Input format for the {@link BbpMapper}. Keys and values represent offsets
	...
	public static class BbpInputFormat extends InputFormat<LongWritable, IntWritable> {
		...
		/** {@inheritDoc} */
		public RecordReader<LongWritable, IntWritable> createRecordReader(InputSplit generic,
	...
	public int run(String[] args) throws IOException {
		...
		if (startDigit <= 0) {
			...
			throw new UnsupportedOperationException("nDigits - 1 + startDigit = " + (nDigits - 1L + startDigit)
					+ " > IMPLEMENTATION_LIMIT + BBP_HEX_DIGITS," + ", where IMPLEMENTATION_LIMIT="
					+ IMPLEMENTATION_LIMIT + "and BBP_HEX_DIGITS=" + BBP_HEX_DIGITS);
		} else if (nMaps <= 0) {
	...
	/**
	 * Limitation of the program. The program may return incorrect results if
	 * the limit is exceeded. The default value is 10^8. The program probably
	 * can handle some higher values such as 2^28.
	 */
	...
	/**
	 * Compute the exact (d+1)th to (d+{@link #BBP_HEX_DIGITS})th hex digits of
	 * pi.
	 */
	...
	/**
	 * Approximate the fraction part of $16^d \sum_{k=0}^\infty
	 * \frac{16^{d-k}}{8k+j}$ for d > 0 and j = 1, 4, 5, 6.
	 */
	...
	/**
	 * Partition input so that the workload of each part is approximately the
	 * same.
	 */

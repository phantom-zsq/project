...
import com.phantom.other.masterslave.command.CommandDispatcher;

/**
 * 后面可以考虑做一个SessionPool
 * 
 * @author 张少奇
 * @time 2017年1月11日 下午2:54:46
 */
public class DefaultSessionManager implements SessionManager {
	...
	public void init() {
		...
		sessionRecycleScheduler.scheduleAtFixedRate(new SessionRecycleTask(), sessionRecycleInterval,
				sessionRecycleInterval, TimeUnit.SECONDS);
	}
	...
	public Session newSession(SocketChannel channel) {
		...
		log.info(new StringBuilder("New session for ").append(channel.socket().getRemoteSocketAddress().toString())
				.toString());
	...
	public void freeSession(Session session) {
		...
		log.warn(new StringBuilder("Free session : id = ").append(session.getId()).append(", for : ")
				.append(session.getRemoteIP()).toString());
	...
	private void dump() {
		...
		while (iterator.hasNext()) {
			...
			if (session != null) {
				log.debug(new StringBuilder("Session: ").append(session.getId()).append(" , State: ")
